...
Basically, This is an API for hymn apps, see list of available hymns below.
...
The database is just a JSON file generated by parsing each text file in the **Hymns** directory. Each file represents a hymns. Let's have a look at two of those files
...
Watching for that pattern, a huge JSON file`(db.json)` is generated to represent hymns. This serves as the database. You can take a look of `makeDB.js` to see how `db.json` is generated. `categories.js` contains all categories and associated hymn numbers. This is also used while generating `db.json`

You can also copy the `db.json` and use it directly in your application

## Contributions

If you run in any bug using this plese feel free to open an issue or catch me on twitter www.twitter.com/marvinjudehk
